/*! angular-breadcrumb - v0.4.1-dev-2016-05-18
...
var registerWatchers = function(labelWatcherArray, interpolationFunction, viewScope, step) {
    angular.forEach(getExpression(interpolationFunction), function(expression) {
        labelWatcherArray.push(viewScope.$watch(expression, function() {
...
function BreadcrumbDirective($interpolate, $breadcrumb, $rootScope) {
    var $$templates = {
        bootstrap2: '<ul class="breadcrumb">' +
            ...
            '<a ng-if="!($last || !!step.abstract)" ui-sref="{{step.ncyBreadcrumbStateRef}}">{{step.ncyBreadcrumbLabel}}</a>' +
            '<span ng-if="$last || !!step.abstract">{{step.ncyBreadcrumbLabel}}</span>' +
